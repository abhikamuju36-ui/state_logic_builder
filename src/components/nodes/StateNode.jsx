...
const SHAPE_POINTS = {
  ...
  pentagon:   '12,0 88,0 100,62 50,100 0,62',
  hexagon:    '10,0 90,0 100,50 90,100 10,100 0,50',
  octagon:    '20,0 80,0 100,20 100,80 80,100 20,100 0,80 0,20',
  decagon:    '15,0 85,0 100,17 100,50 100,83 85,100 15,100 0,83 0,50 0,17',
  dodecagon:  '12,0 88,0 100,12 100,37 100,63 100,88 88,100 12,100 0,88 0,63 0,37 0,12',
  poly14:     '10,0 90,0 100,10 100,30 100,50 100,70 100,90 90,100 10,100 0,90 0,70 0,50 0,30 0,10',
  poly16:     '8,0 92,0 100,8 100,25 100,42 100,58 100,75 100,92 92,100 8,100 0,92 0,75 0,58 0,42 0,25 0,8',
};
...
function ActionRow({ action, devices, onClickName }) {
...
function HomeRow({ device }) {
  ...
  const opLabel = opInfo?.label ?? device.homePosition;
...
function InlinePicker({ smId, nodeId, devices, onClose, editActionId, editAction }) {
  ...
  return (
    <div className="inline-picker" ref={pickerRef}>
      ...
      {step === 'device' && (
        <>
          ...
          {(() => {
            ...
            const hasWaitDecision = visionSignals.length > 0 || projectSignals.length > 0;
            if (!hasWaitDecision) return null;
      ...
      {/* ── Servo position step (absolute positions only) ─────────────── */}
      {step === 'position' && selectedDevice && (() => {
        ...
        return (
          <>
            ...
            <button className="inline-picker__back" onClick={() => { setStep('operation'); }}>
              ← Back
            </button>
          </>
        );
      ...
      {step === 'servoIncrConfig' && selectedDevice && (
        <>
          ...
          <div className="verify-done-row">
            <button className="inline-picker__back" onClick={() => setStep('operation')}>← Back</button>
      ...
      {showPicker && sm && (
        <NodeToolbar isVisible position="right" offset={8}>
          <InlinePicker
            ...
            nodeId={id}
            devices={devices}
            onClose={() => { setShowPicker(false); setEditingActionId(null); }}
              ...
              <ActionRow key={action.id} action={action} devices={devices}
                onClickName={(e) => {
                  ...
                  if (editingActionId === action.id && showPicker) {
                    setShowPicker(false); setEditingActionId(null);
                  } else {
                    setEditingActionId(action.id); setShowPicker(true);
                  }
                }}
              />
            ))
