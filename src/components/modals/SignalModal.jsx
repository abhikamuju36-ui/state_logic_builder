...
export function SignalModal({ isOpen, onClose, signal }) {
  ...
  function handleSave() {
    ...
    if (type === 'position') {
      ...
      const stateName = selectedStateNode ? nodeLabel(selectedStateNode) : cleanName;
      const stateNumber = selectedStateNode?.data?.stepNumber ?? selectedStateNode?.data?.stateNumber ?? null;
